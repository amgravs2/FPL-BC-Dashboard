...
function FixtureDifficultyGrid({ fixtures, players, teams, currentGw }) {
  ...
  gws.forEach(gw => {
    fixtures.filter(f => f.gw === gw).forEach(f => {
      ...
      teamFixtures[f.team_h][gw] = {
        opponent: f.team_a, home: true,
        team_h_attack_fdr: f.team_h_attack_fdr,
        team_h_defence_fdr: f.team_h_defence_fdr,
      };
      teamFixtures[f.team_a][gw] = {
        opponent: f.team_h, home: false,
        team_a_attack_fdr: f.team_a_attack_fdr,
        team_a_defence_fdr: f.team_a_defence_fdr,
      };
    });
  });
    ...
    <div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.8rem' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border-gold)' }}>
              ...
              {gws.map(gw => (
                <th key={gw} colSpan={2} style={{ padding: '0.5rem 0.5rem', textAlign: 'center', color: 'var(--gold-bright)', fontWeight: 600, fontSize: '0.7rem', minWidth: 80, borderLeft: '1px solid var(--border)' }}>
                  GW{gw}
                  <div style={{ display: 'flex', gap: 2, justifyContent: 'center', marginTop: 2 }}>
                    <span style={{ fontSize: '0.55rem', color: '#c07a5a' }}>ATK</span>
                    <span style={{ fontSize: '0.55rem', color: '#5a9e64' }}>DEF</span>
                  </div>
                </th>
              ))}
            </tr>
              ...
              return (
                ...
                >
                  ...
                  {gws.map(gw => {
                    ...
                    if (!fix) return (
                      <td key={gw} colSpan={2} style={{ padding: '0.25rem', textAlign: 'center', borderLeft: '1px solid var(--border)' }}>
                        <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>BGW</div>
                      </td>
                    );

                    const atkKey = fix.home ? 'team_h_attack_fdr'  : 'team_a_attack_fdr';
                    const defKey = fix.home ? 'team_h_defence_fdr' : 'team_a_defence_fdr';
                    const atkFdr = fdrColor(fix[atkKey] || 3);
                    const defFdr = fdrColor(fix[defKey] || 3);

                    return (
                      <React.Fragment key={gw}>
                        <td style={{ padding: '0.2rem 0.15rem', textAlign: 'center', borderLeft: '1px solid var(--border)' }}>
                          <div style={{ background: atkFdr.bg, border: `1px solid ${atkFdr.text}44`, borderRadius: 3, padding: '0.15rem 0.25rem', fontSize: '0.62rem', color: atkFdr.text, fontWeight: 600, whiteSpace: 'nowrap' }}>
                            {opp?.short_name || '?'}{fix.home ? ' H' : ' A'}
                          </div>
                        </td>
                        <td style={{ padding: '0.2rem 0.15rem', textAlign: 'center' }}>
                          <div style={{ background: defFdr.bg, border: `1px solid ${defFdr.text}44`, borderRadius: 3, padding: '0.15rem 0.25rem', fontSize: '0.62rem', color: defFdr.text, fontWeight: 600, whiteSpace: 'nowrap' }}>
                            {opp?.short_name || '?'}{fix.home ? ' H' : ' A'}
                          </div>
                        </td>
                      </React.Fragment>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', marginTop: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.65rem', color: 'var(--text-muted)' }}>
          <span style={{ color: '#c07a5a' }}>ATK</span> = how hard it is to score &nbsp;·&nbsp;
          <span style={{ color: '#5a9e64' }}>DEF</span> = how hard it is to keep a clean sheet
        </div>
      </div>
    </div>
  );
